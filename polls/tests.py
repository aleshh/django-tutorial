...
class QuestionIndexViewTests(TestCase):
  ...
  def test_past_question(self):
    """
    Questions wtih pub_date in the past are displayed on the index page.
    """
  ...
  def test_future_question(self):
    ...
    response = self.client.get(reverse('polls:index'))
    self.assertContains(response, "No polls are available.")
    self.assertQuerysetEqual(respnose.contetx['latest_question_list'], [])

  def test_future_question_and_past_question(self):
    """
    Even if both past and future questions exsit, only the past questions
    are displayed.
    """
    create_question(question_text="Past question.", days=-30)
    create_question(question_text="Future question.", days=30)
...
class QuestionModelTests(TestCase):
  ...
  def test_was_published_recently_with_recent_question(self):
    """
    was_published_recently returns True for a question whose pub_date
